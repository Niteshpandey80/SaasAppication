...
const BlogTitles = () => {
  const blogCategories = [
    ...
    const [selectedCategory , setSelectedCatgory] = useState('General')
          ...
          <div className='flex-col items-center gap-3'>
            ...
            <div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
              {blogCategories.map((item )=> (
                <span onClick={()=> setSelectedCatgory(item)} className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedCategory === item? 'bg-purple-50 text-purple-700' : 'text-gray-500 border-gray-300'} `} key={item}>{item}</span>
              ))}
            </div>
            <br />
